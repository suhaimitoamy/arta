...
const dom = {
  ...
  addMediaFolderBtn: document.querySelector("#addMediaFolderBtn"),
  mediaFolderPills: document.querySelector("#mediaFolderPills"),
  moveFolderDialog: document.querySelector("#moveFolderDialog"),
  moveFolderSelect: document.querySelector("#moveFolderSelect"),
  confirmMoveFolderBtn: document.querySelector("#confirmMoveFolderBtn"),
  moveFolderItemId: document.querySelector("#moveFolderItemId"),
  ...
  journalTimeInput: document.querySelector("#journalTimeInput"),
  journalMarketInput: document.querySelector("#journalMarketInput"),
  journalTitleInput: document.querySelector("#journalTitleInput"),
  journalSessionInput: document.querySelector("#journalSessionInput"),
  journalDirectionInput: document.querySelector("#journalDirectionInput"),
  journalSetupInput: document.querySelector("#journalSetupInput"),
  journalResultInput: document.querySelector("#journalResultInput"),
  journalEntryReasonInput: document.querySelector("#journalEntryReasonInput"),
  journalEntryConfirmationInput: document.querySelector("#journalEntryConfirmationInput"),
  journalPoiInput: document.querySelector("#journalPoiInput"),
  journalTfAnalysisInput: document.querySelector("#journalTfAnalysisInput"),
  journalTfEntryInput: document.querySelector("#journalTfEntryInput"),
  journalEntryPriceInput: document.querySelector("#journalEntryPriceInput"),
  journalLotInput: document.querySelector("#journalLotInput"),
  journalStopLossInput: document.querySelector("#journalStopLossInput"),
  journalTakeProfitInput: document.querySelector("#journalTakeProfitInput"),
  journalRiskInput: document.querySelector("#journalRiskInput"),
  journalRrInput: document.querySelector("#journalRrInput"),
  ...
  journalWhatWentRightInput: document.querySelector("#journalWhatWentRightInput"),
  journalWhatWentWrongInput: document.querySelector("#journalWhatWentWrongInput"),
  journalLessonsInput: document.querySelector("#journalLessonsInput"),
  journalFurtherNotesInput: document.querySelector("#journalFurtherNotesInput"),
  journalEmotionInput: document.querySelector("#journalEmotionInput"),
  journalEmotionBeforeInput: document.querySelector("#journalEmotionBeforeInput"),
  journalEmotionFloatingInput: document.querySelector("#journalEmotionFloatingInput"),
  journalEmotionAfterInput: document.querySelector("#journalEmotionAfterInput"),
...
const MEDIA_FOLDERS_KEY = "arta_media_folders_v1";

function loadMediaFolders() {
  try {
    const parsed = JSON.parse(localStorage.getItem(MEDIA_FOLDERS_KEY) || "[]");
...
function saveMediaFolders() {
  localStorage.setItem(MEDIA_FOLDERS_KEY, JSON.stringify(state.mediaFolders));
}

function renderMediaFolderPills() {
  if (!dom.mediaFolderPills) return;
  dom.mediaFolderPills.replaceChildren();
  
  const createPill = (name, filterValue) => {
    const btn = document.createElement("button");
    btn.type = "button";
    btn.className = "pill-button" + (state.mediaFolderFilter === filterValue ? " is-active" : "");
    btn.textContent = name;
    btn.addEventListener("click", () => {
      state.mediaFolderFilter = filterValue;
  ...
  dom.mediaFolderPills.append(createPill("Semua Media", "Semua"));
  dom.mediaFolderPills.append(createPill("Tanpa Folder", "Tanpa Folder"));
  
  state.mediaFolders.forEach((folder) => {
    dom.mediaFolderPills.append(createPill(folder, folder));
  });
}

function initMediaFolders() {
  dom.addMediaFolderBtn?.addEventListener("click", () => {
    const name = window.prompt("Nama folder baru:");
    if (!name || !name.trim()) return;
    const clean = name.trim();
    if (!state.mediaFolders.includes(clean)) {
      state.mediaFolders.push(clean);
      saveMediaFolders();
      renderMediaFolderPills();
    }
  });

  dom.confirmMoveFolderBtn?.addEventListener("click", () => {
    const id = dom.moveFolderItemId.value;
    const target = dom.moveFolderSelect.value;
...
function openMoveFolderDialog(itemId) {
  if (!dom.moveFolderDialog) return;
  const item = state.items.find(i => i.id === itemId);
  if (!item) return;
  dom.moveFolderItemId.value = itemId;
  
  dom.moveFolderSelect.replaceChildren();
  const emptyOpt = document.createElement("option");
  emptyOpt.value = "";
  emptyOpt.textContent = "Tanpa Folder";
  dom.moveFolderSelect.append(emptyOpt);
  
  state.mediaFolders.forEach(folder => {
    const opt = document.createElement("option");
    opt.value = folder;
    opt.textContent = folder;
    dom.moveFolderSelect.append(opt);
  });
  
  dom.moveFolderSelect.value = item.collection || "";
...
async function boot() {
  ...
  loadSettings();
  state.mediaFolders = loadMediaFolders();
...
function getRenderData() {
  ...
  const mediaItems = allMatches
    ...
    .filter((item) => {
      if (state.mediaFolderFilter === "Semua") return true;
      if (state.mediaFolderFilter === "Tanpa Folder") return !item.collection || item.collection.trim() === "";
      return item.collection === state.mediaFolderFilter;
    });
...
function createCard(item) {
  ...
  const moveFolderAction = card.querySelector(".move-folder-action");
  ...
  if (moveFolderAction) {
    if (isMediaItem(item) || isDocumentItem(item)) {
      moveFolderAction.addEventListener("click", () => {
        openMoveFolderDialog(item.id);
        toggleCardMenu(cardMenu);
      });
    } else {
      moveFolderAction.hidden = true;
    }
  }
...
function getJournalFormData() {
  ...
  const draft = {
    ...
    time: dom.journalTimeInput?.value || "",
    title: dom.journalTitleInput.value.trim() || "Jurnal Trading",
    market: dom.journalMarketInput.value.trim(),
    session: dom.journalSessionInput?.value || "",
    direction: dom.journalDirectionInput?.value || "",
    setup: dom.journalSetupInput.value.trim(),
    result: dom.journalResultInput.value,
    entryReason: dom.journalEntryReasonInput?.value.trim() || "",
    entryConfirmation: dom.journalEntryConfirmationInput?.value.trim() || "",
    poi: dom.journalPoiInput?.value.trim() || "",
    tfAnalysis: dom.journalTfAnalysisInput?.value.trim() || "",
    tfEntry: dom.journalTfEntryInput?.value.trim() || "",
    entryPrice: dom.journalEntryPriceInput?.value || "",
    lot: dom.journalLotInput?.value || "",
    stopLoss: dom.journalStopLossInput?.value || "",
    takeProfit: dom.journalTakeProfitInput?.value || "",
    risk: dom.journalRiskInput?.value.trim() || "",
    rr: dom.journalRrInput?.value.trim() || "",
    ...
    whatWentRight: dom.journalWhatWentRightInput?.value.trim() || "",
    whatWentWrong: dom.journalWhatWentWrongInput?.value.trim() || "",
    lessons: dom.journalLessonsInput.value.trim(),
    furtherNotes: dom.journalFurtherNotesInput?.value.trim() || "",
    emotion: dom.journalEmotionInput.value.trim(),
    emotionBefore: dom.journalEmotionBeforeInput?.value.trim() || "",
    emotionFloating: dom.journalEmotionFloatingInput?.value.trim() || "",
    emotionAfter: dom.journalEmotionAfterInput?.value.trim() || "",
...
function createJournalCard(journal, expanded = false) {
  ...
  if (journal.time || journal.session || journal.direction) {
    const info = [journal.time, journal.session, journal.direction].filter(Boolean).join(" • ");
    body.append(makeJournalTextBlock("Info", info));
  }
  
  const setupDetails = [];
  if (journal.tfAnalysis) setupDetails.push(`TF Analisis: ${journal.tfAnalysis}`);
  if (journal.tfEntry) setupDetails.push(`TF Entry: ${journal.tfEntry}`);
  if (journal.poi) setupDetails.push(`POI: ${journal.poi}`);
  if (journal.entryConfirmation) setupDetails.push(`Konfirmasi: ${journal.entryConfirmation}`);
  if (setupDetails.length) {
    body.append(makeJournalTextBlock("Detail Setup", setupDetails.join(" • ")));
  }

  if (journal.entryReason) body.append(makeJournalTextBlock("Alasan Entry", journal.entryReason));
  
  const riskDetails = [];
  if (journal.entryPrice) riskDetails.push(`Entry: ${journal.entryPrice}`);
  if (journal.stopLoss) riskDetails.push(`SL: ${journal.stopLoss}`);
  if (journal.takeProfit) riskDetails.push(`TP: ${journal.takeProfit}`);
  if (journal.lot) riskDetails.push(`Lot: ${journal.lot}`);
  if (journal.risk) riskDetails.push(`Risk: ${journal.risk}`);
  if (journal.rr) riskDetails.push(`R:R: ${journal.rr}`);
  if (riskDetails.length) {
    body.append(makeJournalTextBlock("Risk & Result", riskDetails.join(" • ")));
  }

  if (journal.plan) body.append(makeJournalTextBlock("Catatan awal", journal.plan));
  if (journal.evaluation) body.append(makeJournalTextBlock("Evaluasi", journal.evaluation));
  
  const psyDetails = [];
  if (journal.emotionBefore) psyDetails.push(`Sebelum: ${journal.emotionBefore}`);
  if (journal.emotionFloating) psyDetails.push(`Floating: ${journal.emotionFloating}`);
  if (journal.emotionAfter) psyDetails.push(`Setelah: ${journal.emotionAfter}`);
  if (psyDetails.length) {
    body.append(makeJournalTextBlock("Psikologi", psyDetails.join(" • ")));
  }
  if (journal.emotion && psyDetails.length === 0) body.append(makeJournalTextBlock("Emosi", journal.emotion));

  body.append(makeJournalTextBlock("Kesalahan", journal.mistakes));
  if (journal.whatWentRight) body.append(makeJournalTextBlock("Apa yang benar", journal.whatWentRight));
  if (journal.whatWentWrong) body.append(makeJournalTextBlock("Apa yang salah", journal.whatWentWrong));
  body.append(makeJournalTextBlock("Pelajaran", journal.lessons));
  if (journal.furtherNotes) body.append(makeJournalTextBlock("Catatan lanjutan", journal.furtherNotes));
...
function editJournal(id) {
  ...
  if (dom.journalTimeInput) dom.journalTimeInput.value = journal.time || "";
  dom.journalMarketInput.value = journal.market || "";
  dom.journalTitleInput.value = journal.title || "";
  if (dom.journalSessionInput) dom.journalSessionInput.value = journal.session || "";
  if (dom.journalDirectionInput) dom.journalDirectionInput.value = journal.direction || "";
  dom.journalSetupInput.value = journal.setup || "";
  dom.journalResultInput.value = journal.result || "Belum selesai";
  
  if (dom.journalEntryReasonInput) dom.journalEntryReasonInput.value = journal.entryReason || "";
  if (dom.journalEntryConfirmationInput) dom.journalEntryConfirmationInput.value = journal.entryConfirmation || "";
  if (dom.journalPoiInput) dom.journalPoiInput.value = journal.poi || "";
  if (dom.journalTfAnalysisInput) dom.journalTfAnalysisInput.value = journal.tfAnalysis || "";
  if (dom.journalTfEntryInput) dom.journalTfEntryInput.value = journal.tfEntry || "";
  
  if (dom.journalEntryPriceInput) dom.journalEntryPriceInput.value = journal.entryPrice || "";
  if (dom.journalLotInput) dom.journalLotInput.value = journal.lot || "";
  if (dom.journalStopLossInput) dom.journalStopLossInput.value = journal.stopLoss || "";
  if (dom.journalTakeProfitInput) dom.journalTakeProfitInput.value = journal.takeProfit || "";
  if (dom.journalRiskInput) dom.journalRiskInput.value = journal.risk || "";
  if (dom.journalRrInput) dom.journalRrInput.value = journal.rr || "";
  ...
  if (dom.journalWhatWentRightInput) dom.journalWhatWentRightInput.value = journal.whatWentRight || "";
  if (dom.journalWhatWentWrongInput) dom.journalWhatWentWrongInput.value = journal.whatWentWrong || "";
  dom.journalLessonsInput.value = journal.lessons || "";
  if (dom.journalFurtherNotesInput) dom.journalFurtherNotesInput.value = journal.furtherNotes || "";
  dom.journalEmotionInput.value = journal.emotion || "";
  if (dom.journalEmotionBeforeInput) dom.journalEmotionBeforeInput.value = journal.emotionBefore || "";
  if (dom.journalEmotionFloatingInput) dom.journalEmotionFloatingInput.value = journal.emotionFloating || "";
  if (dom.journalEmotionAfterInput) dom.journalEmotionAfterInput.value = journal.emotionAfter || "";
